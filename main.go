...
func main() {
	...
	bot.HandleMessage("", func(m *tbot.Message) { //The AI must be triggered by the keyword "human:"
		c.SendMessage(m.Chat.ID, ""+getChatGPTresponse(ctx, m.Text)) //m.Text represents the text you typed in 代表你打的文字
	})
	log.Fatal(bot.Start())
}
